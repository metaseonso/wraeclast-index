...
The artifact page (gems, uniques, passive tree) is the drill-down. This script:
  ...
  3. builds data/index.json, the compact search index (with the base items, the Atlas and the currency the catalogue
     ...
     and our interaction cards, tools/interactions.py), marks the phrases in each card's lines that name another card
     (tools/nodelinks.py), counts every interaction the game's wording names (data/interactions.json), and writes its
...
def build_index(html):
    ...
    # our own cards: the mechanics the game never writes down (tools/mechanics.py), and the interactions its
    # wording names that nobody has settled (tools/interactions.py). Not game text, and they say so.
    import mechanics
    ...
    # every interaction the game's wording names, settled into one bucket each, counted and reported
    rep = interactions.survey(index)
    interactions.write(index, rep)
    interactions.report(rep)
    return index
