...
| **A number that moves, beside one that does not** | Two statements, two slots, never one sentence. A price carries its own age and is gone rather than stale; a fact carries its source where the game does not publish it; and a reader can take either one without the other having to be true. Welded together they read as a single claim that is half true by the hour — *"a Perfect Exalted Orb costs 1,245 plain ones and what it buys is modifier level 50"* is a price and a game fact wearing one sentence. The price goes through the `money` type, which is the only thing on the site that draws a price, and the only thing that knows how to say how old it is. |
