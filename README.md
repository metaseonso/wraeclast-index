...
`gameinfo.py`, `atlas.py`, `bosses.py`, `farms.py` and `guides.py` (where the outside source is a link we
send players to, and the fault is that it has rotted). The tools that build only from files already on disk
