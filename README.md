...
| `data/bosses.json`, `data/bossqueries.json` | Bosses: every endgame boss, what it drops and what it costs to fight, by hand with `python tools/bosses.py`; the entry items the in-game Currency Exchange does not trade get a trade search of their own here. The site serves `/data/bossprices.json`: every one of those items' real price, from the hourly unique checks, the Currency Exchange and those searches (`worker/prices.js`) |
